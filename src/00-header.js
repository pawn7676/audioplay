...
(function () {
   ...
   *
   *    w20  THE REBUILD on shared v134 sections, as above.
   *         VERSION is reassigned in 11-lichess (section 1 is
   *         shared and byte-frozen until a joint bump).
   *         Gains over w19, inherited from v105-v134 in
   *         one step: the settings panel (v124) with
   *         per-mode read-back and the message-channel
   *         invariant (v129/v130), guarded pawn pushes,
   *         the tags and channel vocabulary, clock mode as
   *         canon section 14, the v134 read-back race fix,
   *         and every parser fix since v104. Losses, on
   *         purpose: silent mode (deleted from canon at
   *         v109), the voice dropdown (the shared pipeline
   *         uses the SYSTEM voice - see VOICE_NAME in
   *         section 1 - which is also what picks up
   *         downloaded Premium voices), and the w2
   *         low-time callouts (not in canon; reopen from
   *         the tombstone in reference/modes.js if
   *         missed). Started on v133 sections; re-cut to
   *         v134 the day v134 landed, the first proof the
   *         re-copy discipline works.
   *         UNPROVEN: no real game has been played on w20.
   *    w21  THE FIRST DEVICE FINDINGS, from the owner's
   *         first look at w20 on the iPad. Three fixes:
   *         (1) the button row joins the page - floating
   *         bottom-right was shaped for lichess.org and
   *         covered the page's own content here; it now
   *         lives in a Voice panel at the top, and the
   *         settings panel opens BELOW its button. buildUI
   *         stays verbatim; the re-parent is a web delta.
   *         (2) the page's button CSS is SCOPED to .panel:
   *         bare selectors were bleeding flex and padding
   *         into the shared UI's created buttons, which no
   *         page CSS could ever reach on lichess.org - the
   *         on/off pills came out different sizes. (3) the
   *         turn line's "- that is you" is gone at the
   *         owner's word: the board is drawn from your side
   *         and the clocks say you/them already.
   *         PROVEN the same day: the first real game on the
   *         website (game w21-1, a win by Qxf7#, log in the
   *         project files). Sign-in, PKCE, the account event
   *         stream, the game stream, mic, clock mode with a
   *         HELD wake lock, live settings flips mid-game,
   *         and the v134 read-back race fix all seen working
   *         on device. Zero retries in the entire game.
   *    w22  INSTRUCTIONS BECOME A PANEL. The four hint
   *         paragraphs leave the Lichess panel for their
   *         own collapsible "Instructions" panel at the
   *         bottom of the page - open by default so a
   *         first visitor still meets them, remembered
   *         like every panel so the owner collapses them
   *         once and they stay down. The sign-in paragraph
   *         is reworded to the owner's text ("allow this
   *         site to do 3 things on your behalf...").
   *    w23  the "SET loaded:" boot line, inherited from
   *         v135 - the first shared-tree change to flow
   *         into both targets from one edit. See v135 in
   *         us-header.js for the reasoning.
   *    w24  the settings panel opens UNDER ITS BUTTON, both
   *         axes. The userscript's right:10px pin was the
   *         button's own corner there; here it read as a
   *         random spot on the right. Also the session's
   *         log proved two more things in passing: the
   *         account event stream RECONNECTED after a real
   *         "Load failed" drop (18:14:42-45), and the v129
   *         message-channel invariant fired live, forcing
   *         show-messages back on when both were switched
   *         off.
   *    w25  no double-tap zoom on the overlays: two quick
   *         taps on two settings pills zoomed the page. The
   *         w21 CSS scoping that fixed the pill sizes also
   *         took touch-action:manipulation away from the
   *         body-attached panels; restored inline on both
   *         panels and their buttons. If the owner ever
   *         reports the same zoom on lichess.org, this
   *         belongs in the shared section 12 as a v-bump -
   *         the pills never carried touch-action there
   *         either, lichess's own CSS just may have covered
   *         it.
   *    w26  cancel closes a repair question, from v136 in
   *         the shared section 6 - the second shared-tree
   *         change to reach both targets from one edit.
   *         PROVEN THE SAME SESSION: the website's second
   *         full game, 45 minutes against maia5, won by
   *         underpromotion (c1=B#). Read the log for what
   *         the parser now absorbs without a retry -
   *         "Fisher hotel five" for Bh5, "Bush golf six"
   *         for g6, "Push Charlie to" for c2, and the
   *         first-word demotion firing thirteen times.
   *    w27  the voice row is REVERSED: round button first,
   *         then settings, clock, log, practice. Done with
   *         row-reverse, not by reordering buildUI - the
   *         shared UI stays re-copyable. Note the settings
   *         panel still anchors to its button (w24), which
   *         now sits further right; the clamp keeps it on
   *         screen.
   *    w28  w27 DID NOTHING and shipped anyway. The flip
   *         went on wrapEl, whose only child is the row
   *         that really holds the buttons - reversing a list
   *         of one. It is on the inner row now. The test
   *         that passed had grepped the SOURCE for
   *         "row-reverse" instead of asking the DOM what
   *         got styled; it now walks the built tree and
   *         checks the element that contains the buttons.
   *         A test that cannot fail when the feature is
   *         broken is worse than no test: it spends the
   *         owner's trust to prove nothing.
   *    w29  the round button becomes a labelled pill:
   *         "start voice" in the page's primary blue when
   *         off, "listening"/"voice on" in green when on.
   *         The circle was the userscript's thumb target
   *         floating over lichess.org; in a panel it only
   *         forced a 72px row with gaps around every button.
   *         It stays visibly the must-press control, but
   *         by wearing the same blue as "Sign in with
   *         Lichess" and saying so in words rather than by
   *         being three times the size of its neighbours.
   *    w30  every button label capitalised - Practice, Log,
   *         Clock, Settings, Copy, Clear, Close - matching
   *         the page's own buttons, which always were. The
   *         voice button says "Start" alone: the panel
   *         heading above it already says VOICE, so
   *         repeating the word was the label doing the
   *         panel's job.
   *    w31  Start goes first by MOVING THE NODE, not by
   *         reversing the row. row-reverse (w27) looked
   *         right on the iPad and broke on an iPhone in
   *         portrait: a reversed row wraps in reverse too,
   *         so Start fell to the bottom line. The lesson
   *         is the one the whole website keeps relearning
   *         - a layout trick that happens to look right at
   *         ONE width is not a layout. Now the DOM order
   *         is the reading order at every width.
   *    w32  three things the owner asked for on seeing the
   *         page whole: the voice buttons now match the
   *         page's own buttons (the shared UI's inline
   *         sizing is CLEARED so the stylesheet decides,
   *         rather than a second set of numbers to keep in
   *         step); the "Audioplay" title is gone, since the
   *         tab already says it and the first screen is
   *         worth more; and Instructions starts COLLAPSED,
   *         because it is the longest panel and it is
   *         reference, not something to read past on the
   *         way to a game. Carries v137's rename - they are
   *         buttons, not "chips". Dead w19 CSS for buttons
   *         this page never builds went too.
   *    w33  TIME CONTROLS ARE PRESET BUTTONS - the pairs
   *         from Lichess's own quick-pairing grid, minus
   *         bullet (a spoken move costs seconds a bullet
   *         game does not have; the owner doubts blitz too,
   *         and it stays until a real game settles it).
   *         3+0 through 30+20 plus a Custom box that takes
   *         the same #+# and selects itself when the text
   *         parses. The choice is remembered; seek and
   *         challenge read selectedTimeControl(), never the
   *         DOM. Also the session the FREEZE was decided -
   *         see the notice at the top of this header.
   *    w34  no default time control, and no memory of the
   *         last one: the row is clean on every load and
   *         picking is one deliberate tap. w33 had both a
   *         15+10 default and a remembered choice, which
   *         are the same thing wearing two hats - either
   *         one lights a button the owner did not press
   *         this session. Seeking or challenging with
   *         nothing picked says "Pick a time control
   *         first."; with a Custom box that does not parse
   *         it says that instead. Neither sends anything.
   *    w35  the memory comes back, the default does not.
   *         The owner's distinction, and w34 collapsed it:
   *         a FIRST visit picks nothing, because the page
   *         has no business guessing; a LATER visit
   *         restores what was chosen, because you already
   *         said. Null now means "not chosen yet" and
   *         nothing else. Junk in storage reads as never
   *         chosen - a stale value should leave a clean
   *         row, not a broken one.
   *    w36  the Custom box was UNREADABLE when picked:
   *         black browser-default text on the picked
   *         green, because the box had no colour of its
   *         own and w33 set only a background, inline. It
   *         is now styled like the presets beside it -
   *         same surface, text, border - and "picked" is
   *         the same CSS class for all ten controls
   *         instead of inline colours on one of them. The
   *         general rule this keeps proving: state set
   *         inline from code cannot see what the
   *         stylesheet already decided.
   *    w37  a custom time could not be chosen TWICE. The
   *         box only re-picked on "input", which fires
   *         when the text changes - so tapping back into a
   *         box that already said 40+30, after pressing a
   *         preset, gave a cursor and stayed dark with no
   *         way back to green short of retyping. Focus is
   *         the missing event. Found by the owner in
   *         thirty seconds of use; the w33 tests had only
   *         ever TYPED into the box, never returned to it.
   *         Worth remembering when writing the next test:
   *         a feature used twice is a different feature.
   *    w38  THE SHADOWS ARE GONE. src/ held two copies of
   *         sections 11, 12 and 15 - the userscript's and
   *         the web-* files that actually built - which was
   *         the sharing arrangement's last remnant and an
   *         invitation to edit the wrong one. The
   *         userscript's four sources moved to
   *         frozen-userscript/ beside its canon artifact
   *         and sha; the web-* files took the plain names.
   *         One manifest, one target, one file per section.
   *         The template's own header was rewritten too:
   *         it still pointed at us-header as the front
   *         door, and it still carried the w11 rule that
   *         w36 deliberately broke - see the amendment
   *         there for who owns state appearance now.
   *    w39  spelled for the ear, named for the board.
   *         Two of the owner's, from listening and from
   *         reading: (1) every English voice says
   *         "LITCH-ess", so the one SPOKEN mention is now
   *         "lee chess" - spoken text is spelled the way
   *         it should sound, and the name stays correct
   *         everywhere it is read; (2) the clocks and the
   *         turn line say White and Black, capitalised,
   *         White first as on any score sheet, instead of
   *         "you" and "them" - the colours are what the
   *         game is about, and you/them made the reader
   *         translate twice. Which clock is YOURS is still
   *         marked by colour, so naming them properly cost
   *         nothing.
   *================================================================*/
