/*  audioplay-web — lichess.js
 *  Read the app.js header first: it carries the project
 *  story, the hard constraints, and the file map.
 *
 *  Section 11 of the userscript, ported with its one real
 *  rework. Everything that talks to Lichess is here: the
 *  game stream, the polling fallback, move posting, offer
 *  handling, the clocks — all verbatim — plus the two
 *  things the website changes:
 *
 *  1. THE TOKEN. The userscript kept it in the Userscripts
 *     app's storage because localStorage belonged to
 *     lichess.org, readable by anything on that page. On
 *     our own origin localStorage belongs to US, so that
 *     whole apparatus retires. Better: nobody creates a
 *     token by hand any more. "Sign in with Lichess" runs
 *     the OAuth PKCE flow (ported from BoardEye), entirely
 *     in the browser — no server, which is what lets this
 *     live on GitHub Pages. The user taps, approves once
 *     on lichess.org, and is returned here signed in.
 *
 *  2. THE GAME. There is no lichess.org URL to read a game
 *     id from. Instead /api/stream/event is watched after
 *     sign-in, and a gameStart event joins the game — so a
 *     game begun ANYWHERE (this page's seek or challenge
 *     buttons, the Lichess app, a friend's challenge)
 *     is picked up the moment it exists.
 *
 *  The seek and challenge calls are BoardEye's, ported.
 */
  ...
  /* ---- token: localStorage + OAuth PKCE (from BoardEye) ---- */

  // On lichess.org, localStorage was off-limits: it belongs
  // to the site and every extension on it. On our own
  ...
  function syncMoves(uciString, announce) {
    ...
    for (var i = api.moves.length; i < list.length; i++) {
      ...
      log("MOV", (moverIsMine ? "me   " : "opp  ") + list[i] + " = " + res.san +
          (announce ? "" : " (catch-up)"));
      // silent mode: the move is already in the upper-right
      // quadrant, so announcing it would only misfile the
      // text into the lower-right info area
      if (announce && !moverIsMine && !silentModeOn()) {
    ...
    apiGet("/api/account/playing?nb=10").then(function (d) {
      ...
      if (g.lastMove && api.moves[api.moves.length - 1] !== g.lastMove) {
        ...
        if (res) {
          ...
          if (res.move.color !== api.myColor && !silentModeOn()) {
            speak(sanToSpeech(res.san) + ".");
          }
  ...
  /* ---- connecting: the event stream replaces the URL ---- */
